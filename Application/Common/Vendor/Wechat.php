...
//创建函数调用易源应用接口
	public function showAPI($paramArr,$showapi_secret) {
		$paraStr = "";
		$signStr = "";
		ksort($paramArr);
	 ...
	 * @param $from 出发站
	 * @param $to 终点站
	 * @param $date 出发时间 可以为空
	 */
	public function callTicket($from,$to,$date) {
		$ch = curl_init();
		//$url = 'http://apis.juhe.cn/train/yp?key=44965a9d80271ca911f53a68476ef41f&dtype=json&from='.$from.'&to='.$to.'&date='.$date;
        $url ='http://apis.juhe.cn/train/s2swithprice?start='.$from.'&end='.$to.'&date='.$date.'&key=44965a9d80271ca911f53a68476ef41f';
        $header = array(
			'apikey: 8cc762e8d5b587dad49bebe5037d1ca5',
		);
		// 添加apikey到header
		...
		/*$showapi_appid = '28378';  //替换此值,在官网的"我的应用"中找到相关值
		$showapi_secret = '52d2ef68a5d74efbbf078348b16e6db0';  //替换此值,在官网的"我的应用"中找到相关值
		$paramArr = array(
		...
		$param = $this->showAPI($paramArr,$showapi_secret);
		$url = 'http://route.showapi.com/832-3?'.$param;
	 ...
	 *  获取最新新闻
	 */
	public function callGeekart() {
		$url = "http://news-at.zhihu.com/api/4/news/latest";
		$data = $this->httpGet($url);
		$data_array = json_decode($data, true);
		$top_stories = $data_array['stories'];
		$messages = array();
		foreach ($top_stories as $key=>$story) {
			$messages[] = array(
				'Title' => $story['title'],
				'Description' => $story['title'],
				'PicUrl' => $story['images'][0],
				'Url' => U("geek/zhihu_details", array('id' => $story['id']),false,true,true)
			);
			if ($key==4) {
				break;
			}
    ...
    public function callScience(){
        $ch = curl_init();
        $url ='http://v.juhe.cn/toutiao/index?type=keji&key=96ca3a1a14a312311f1deb753a79a864';
        $header = array(
            'apikey: 8cc762e8d5b587dad49bebe5037d1ca5',
        );
        // 添加apikey到header
	...
	/*public function callScience($num,$page) {
		$ch = curl_init();
		$url = 'http://apis.baidu.com/txapi/keji/keji?num='.$num.'&page='.$page;
		$header = array(
			'apikey: 8cc762e8d5b587dad49bebe5037d1ca5',
		);
		// 添加apikey到header
	...
	public function callHotmovie() {
		//showAPI 调用最新电影接口
		$showapi_appid = '28378';  //在官网的"我的应用"中找到相关值
		$showapi_secret = '52d2ef68a5d74efbbf078348b16e6db0';  //在官网的"我的应用"中找到相关值
		$paramArr = array(	//添加其他参数
			'showapi_appid'=> $showapi_appid
		);
		$param = $this->showAPI($paramArr,$showapi_secret);
		$url = 'http://route.showapi.com/578-2?'.$param;

		$result = $this->httpGet($url);

		$result = json_decode($result,true);
		$data = $result['showapi_res_body']['datalist'];
		$msg = '';
		foreach ($data as $key=>$value) {
			$msg .= '影名:'.$value['MovieName']." ".'票价:'.$value['AvgPrice']."\n";
		}
		return $msg;
	}
	//qq音乐接口
	public function callHotmusic(){
		$showapi_appid = '28378';  //替换此值,在官网的"我的应用"中找到相关值
		$showapi_secret = '52d2ef68a5d74efbbf078348b16e6db0';  //替换此值,在官网的"我的应用"中找到相关值
		$paramArr = array(
			//每个api的参数
		...
		$result = $this->httpGet($url);
		$result = json_decode($result,true);
		$result = $result['showapi_res_body']['pagebean']['songlist'];
		$hot_music = array();
		foreach ($result as $key=>$music) {
			$hot_music[] = array(
				'Title' => $music['songname'],
				'Description' => $music['songname'],
				'PicUrl' => $music['albumpic_big'],
				'Url' => $music['url'],
			);
			if($key==4){
				break;
	...
	/*public function callHotmusic() {
		...
		$result = json_decode($result,true);
		$data = $result['showapi_res_body']['pagebean']['songlist'];
		$i = 0;
		$msg = '';
		foreach ($data as $key=>$value){
			$i++;
			if($i == 8){
				...
				$msg .= "<a href='$value[url]'>$value[songname]</a>"."--"."$value[singername]"."\n";
			}
		}
		return $msg;
	}*/

	public function callWeather($lat,$lng) {
		$showapi_appid = '28378';  //在官网的"我的应用"中找到相关值
		$showapi_secret = '52d2ef68a5d74efbbf078348b16e6db0';
		$paramArr = array(
			'showapi_appid'=> $showapi_appid,
			'from'=> "5",
			'lng'=> $lng,
		...
		$param = $this->showAPI($paramArr,$showapi_secret);
		$url = 'http://route.showapi.com/9-5?'.$param;

		$result = $this->httpGet($url);

		$result = json_decode($result,true);

		$city = $result['showapi_res_body']['cityInfo']['c5'];
		//$district = $result['showapi_res_body']['cityInfo']['c3'];
		$weather = $result['showapi_res_body']['now']['weather'];
		$contentStr = $city." "."当前:"."$weather";
		$f1 = "今天: "."早:".$result['showapi_res_body']['f1']['day_weather']." ".$result['showapi_res_body']['f1']['day_air_temperature']."℃ "."晚:".
			$result['showapi_res_body']['f1']['night_weather']." ".$result['showapi_res_body']['f1']['night_air_temperature']."℃ ".
			$result['showapi_res_body']['f1']['day_wind_power'];
		$f2 = "明天: "."早:".$result['showapi_res_body']['f2']['day_weather']." ".$result['showapi_res_body']['f2']['day_air_temperature']."℃ "."晚:".
			$result['showapi_res_body']['f2']['night_weather']." ".$result['showapi_res_body']['f2']['night_air_temperature']."℃ ".
			$result['showapi_res_body']['f2']['day_wind_power'];
		$f3 = "后天: "."早:".$result['showapi_res_body']['f3']['day_weather']." ".$result['showapi_res_body']['f3']['day_air_temperature']."℃ "."晚:".
			$result['showapi_res_body']['f3']['night_weather']." ".$result['showapi_res_body']['f3']['night_air_temperature']."℃ ".
			$result['showapi_res_body']['f3']['day_wind_power'];
...
//创建函数调用图灵机器人接口
	public function callTuling($keyword,$userid)
	{
		$apiKey = "73ac5cb85ad04082b6d6e38aab50aaf6"; //填写后台提供的key
		$apiURL = "http://www.tuling123.com/openapi/api?key=KEY&info=INFO&userid=$userid";

		$reqInfo = $keyword;
		$url = str_replace("INFO", $reqInfo, str_replace("KEY", $apiKey, $apiURL));

		$message = $this->httpGet($url);
//获取图灵机器人返回的数据，并根据code值的不同获取到不用的数据
		...
		if ($message['code'] == 100000){
			...
			$result = $text . " " . $url;
		}else if ($message['code'] == 308000){
			$text = $message['text'];
			$name = $message['list'][0]['name'];
			$url = $message['list'][0]['detailurl'];
			$result = $text . " " . $name . " " . $url;
		} else {
			$result = "What are you ? 说啥咧！";
		}
		return $result;
	}
	...
	public function httpPost($url,$post_array) {
		...
		//$post_array = array ("username" => "bob","key" => "12345");
	...
	/**
	 * 给某个指定的用户发送消息
	 * Example: $obj->text('hello','123sdcsd123dfa')->reply();
	 * @param string $text
	 */
	public function text_id($text='',$openid,$time)
	{
		$FuncFlag = $this->_funcflag ? 1 : 0;
		$msg = array(
			'ToUserName' => $openid,
...
<Nonce><![CDATA[%s]]></Nonce>
	...
	protected function setCache($cachename,$value,$expired){
		...
		if(!$token->where("name = '$cachename'")->find()){
			$token->add(['name'=>$cachename,'value'=>$value,'time'=>time()]);
		}else{
			$token->where("name = '$cachename'")->save(['name'=>$cachename,'value'=>$value,'time'=>time()]);
		}
	...
	protected function getCache($cachename){
		...
		$data = $token->where("name = '$cachename'")->find();
	 ...
	 * @return bool
	 */
	function sendMsgToAll($userid,$content){
		if (!$this->access_token && !$this->checkAuth()) return false;
	...
	function send_voice($userid,$MediaId){
		//voice | image | mpvideo => array( "media_id"=>"MediaId")
			if (!$this->access_token && !$this->checkAuth()) return false;
	...
	//客服消息
	 function messageToUserName($content,$fromUsername)//content 就是回复的消息，$fromUsername就是openid
	{
		//这里要获取token
